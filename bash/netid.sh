...
while [ $# -gt 0 ]; do
    ...
    -v)
        ...
        ;;
    *)
        # if more than 1 interface will be passed only the first one will be added to ajajInterface
        if [ "$ajayInterface" == "" ]; then
            ajayInterface="$1"
        fi
        ;;
    esac
...
allInterface=$(ifconfig | cut -d ' ' -f1 | tr ':' '\n' | awk NF)

# Loop to go through all the interfaces
for interface in $allInterface; do
